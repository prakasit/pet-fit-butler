...
export default function GalleryPage() {
  ...
  return (
    <div className="space-y-10">
      ...
      >
        <p className="text-sm text-soft-cream/80">ช่วงเวลาที่บันทึกไว้</p>
        <h2 className="mt-1 text-3xl leading-tight">ภาพความทรงจำและคลิปกิจกรรม</h2>
        <p className="mt-2 text-sm text-soft-cream/80">
          {imageAssets.length} ภาพ · {videoAssets.length} คลิปวิดีโอ
        </p>
      </motion.section>

      <PremiumCard title="แกลเลอรีภาพถ่าย" subtitle={`ภาพตัวอย่าง ${imageAssets.length} รายการ`}>
            ...
            >
              <div className="grid h-40 place-items-center bg-[linear-gradient(135deg,rgba(15,27,45,0.86),rgba(143,175,155,0.85),rgba(233,216,195,0.85))] text-soft-cream">
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm">ภาพที่ {index + 1}</p>
                </div>
              </div>
              <div className="space-y-2 p-3 text-sm">
                <p className="font-semibold text-brand-navy">{asset.title}</p>
                <p className="text-xs text-text-muted">
                  {formatDate(asset.capturedAt)} • {asset.sizeMb} เมกะไบต์
                </p>
                <button
                  type="button"
      ...
      <PremiumCard title="ตัวอย่างวิดีโอ" subtitle={`คลิปกิจกรรม ${videoAssets.length} รายการ`}>
            ...
            >
              <div className="relative grid h-44 place-items-center bg-[linear-gradient(135deg,rgba(15,27,45,0.9),rgba(201,167,100,0.72),rgba(143,175,155,0.72))] text-soft-cream">
                ...
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm">คลิปที่ {index + 1}</p>
                </div>
              </div>
              <div className="space-y-2 p-3 text-sm">
                <p className="font-semibold text-brand-navy">{asset.title}</p>
                <p className="text-xs text-text-muted">
                  {formatDate(asset.capturedAt)} • {asset.sizeMb} เมกะไบต์ • {asset.durationSec} วินาที
                </p>
                <button
                  type="button"
                  className="inline-flex items-center gap-1 rounded-lg bg-brand-navy px-3 py-1.5 text-xs font-semibold text-soft-cream transition hover:-translate-y-0.5"
